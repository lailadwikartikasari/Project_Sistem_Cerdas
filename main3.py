...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def clear_inputs(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            # Pastikan tombol "Classification Report" dinonaktifkan setelah clear
            self.ui.pushButton_classification_report.setEnabled(False)
    ...
    def show_classification_report(self):
        """Menampilkan classification report dalam bentuk heatmap."""
        if self.model is None:
            QMessageBox.warning(self, "Error", "Model belum dilatih. Harap latih model terlebih dahulu!")
            return
        ...
        try:
            ...
            # Konversi report menjadi DataFrame untuk memudahkan plotting
            report_df = pd.DataFrame(report).transpose()

            # Buat heatmap menggunakan Seaborn
            plt.figure(figsize=(12, 8))
            
            # Menggunakan cmap yang lebih bervariasi untuk perbedaan warna lebih jelas
            cmap = sns.diverging_palette(220, 20, as_cmap=True)

            # Format angka lebih presisi dengan fmt=".2f" untuk menunjukkan 2 desimal
            sns.heatmap(report_df, annot=True, cmap=cmap, cbar=True, fmt=".2f", linewidths=0.5)

            # Menambahkan judul dan label sumbu
            plt.title('Classification Report Heatmap')
            plt.yticks(rotation=0)  # Untuk memastikan label y terlihat jelas
            ...
            QMessageBox.critical(self, "Error", f"Gagal menampilkan classification report: {str(e)}")
    ...
    def process(self):
        ...
        if self.data is not None and self.model is not None:
            ...
            try:
                ...
                self.processed = True
                 # Jika semua berjalan lancar, baru aktifkan tombol report
                self.ui.pushButton_classification_report.setEnabled(True)
